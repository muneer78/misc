...
def create_markdown_file(tag, title, imagename, date, body):
    ...
    if not filename_title:
        print(
            f"Skipping file creation: title '{title}' results in an invalid filename."
        )
        return
...
csv_file_path = (
    "/Users/muneer78/Documents/Projects/muneericaposts.csv"  # Update this path
)
...
try:
    with open(csv_file_path, newline="", encoding="utf-8") as csvfile:
        ...
        for row in reader:
            ...
            body = row.get(
                "body", ""
            )  # Get the 'body' column, default to an empty string if not present
