...
def make_outline(title, url):
    created = time.strftime("%a %b %d %Y %H:%M:%S GMT%z (%Z)", time.localtime())
    # htmlUrl is set to the feed url (or you can leave it blank)
    return f'      <outline category="importance/365" created="{created}" text="{title}" xmlUrl="{url}" htmlUrl="{url}" title="{title}"/>\n'

with open("feeds.opml", "w", encoding="utf-8") as f:
    f.write(opml_header)
    for title, url in rss_feeds.items():
        f.write(make_outline(title, url))
    f.write(opml_footer)

print("feeds.opml generated.")