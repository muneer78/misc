...
df = df.with_columns([
    ((buy_initial_payment * ((1 + buy_annual_increase) ** ((pl.col("month") - 1) // 12))).alias("buy_monthly_payment")),
    ((rent_initial_payment * ((1 + rent_annual_increase) ** ((pl.col("month") - 1) // 12))).alias("rent_monthly_payment"))
])

# Calculate the cumulative cost for buying and renting
df = df.with_columns([
    (pl.col("buy_monthly_payment").cum_sum().alias("buy_cumulative_payment") + down_payment),
...
result = df.filter(pl.col("buy_cumulative_payment") <= pl.col("rent_cumulative_payment")).select(["month", "buy_cumulative_payment", "rent_cumulative_payment"])
...
if result.height > 0:
    ...
    print(f"Buying becomes cheaper than renting after {years} years and {months} months.")
else:
