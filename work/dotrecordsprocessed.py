...
# Take all records where Processed column is not null
...
# Find all rows where there are duplicate values in "dot_number__c" column
dot_duplicates_df = df[df.duplicated(subset=["dot_number__c"], keep=False)]

# Drop records from dot_duplicates_df that are in processed_df
...
# Find all rows where there are duplicate values in "mc_number__c" column
mc_duplicates_df = df[df.duplicated(subset=["mc_number__c"], keep=False)]

# Write records from df not in processed_df, dot_duplicates_df, and mc_duplicates_df to "leftovers.csv"
leftovers_df = df[
...
# Drop rows from mc_duplicates_df that are in processed_df
