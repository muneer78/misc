...
def assign_and_filter_leads(df_leads, repdict, rep_id_dict):
    ...
    state_to_country_code = {"AZ": "US", "IL": "US", "GA": "US", "TN": "US", "TX": "US"}
    ...
    df_leads = df_leads.apply(
        assign_reps, args=(repdict, rep_id_dict, assigned_reps), axis=1
    )
...
def update_state_names(df):
    ...
    df["state"] = df["state"].apply(
        lambda x: state_to_code.get(x, x)
    )  # Map state names to state codes if available, else leave as-is
    return df
...
def process_and_export_exceptions(df_opps_error):
    ...
    df_opps_error = df_opps_error.apply(
        assign_reps, args=(repdict, rep_id_dict, assigned_reps), axis=1
    )
