#!/usr/bin/env python3
"""List and run the ebook and EPUB scripts in this directory."""
...
def ebook_scripts() -> list[Path]:
    """Return runnable ebook- and epub-prefixed files, excluding this front end."""
    this_script = Path(__file__).resolve()
    candidates = {
        path.resolve()
        for prefix in ("ebook*", "epub*")
        for path in SCRIPT_DIR.glob(prefix)
        if path.is_file()
...
def select_script(selector: str, scripts: list[Path]) -> Path:
    ...
    raise ValueError(f"no ebook or EPUB script matches '{selector}'")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Choose and run an ebook- or epub-prefixed script from the text folder."
    )
    parser.add_argument(
        "script",
        nargs="?",
...
def main() -> int:
    args = parse_args()
    scripts = ebook_scripts()
    if not scripts:
        print("No runnable ebook- or epub-prefixed scripts found.", file=sys.stderr)
        return 1
    ...
    if selector is None:
        if not sys.stdin.isatty():
            ...
            return 2
        print("Available ebook and EPUB scripts:")
        print_scripts(scripts)
        try:
