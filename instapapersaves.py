...
data = [{'URL': match[0], 'Title': match[1], 'Website Title': match[0].split('//')[1].split('/')[0]} for match in url_matches]

# Create a Pandas DataFrame
df = pd.DataFrame(data)

# Remove "www" and ".com" if they exist in the "Website Title" column
df['Website Title'] = df['Website Title'].str.replace('www.', '').str.replace('.com', '')
