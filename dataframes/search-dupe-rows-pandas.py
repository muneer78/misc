...
if not df1_null_contact_id.empty:
    df1_null_contact_id.to_excel(
        writer, sheet_name="Regional Without Contact IDs", index=False
    )

# Check if df2_null_contact_id has data before writing it
if not df2_null_contact_id.empty:
    df2_null_contact_id.to_excel(
        writer, sheet_name="Language Without Contact IDs", index=False
    )
