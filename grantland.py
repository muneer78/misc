...
with open('grantland_features_urls.csv', mode='w', newline='') as file:
    ...
    while current_url:
        ...
        if response.status_code != 200:
            print(f"Failed to retrieve {current_url} with status code: {response.status_code}")
            break
        ...
        for link in soup.find_all('a', href=True):
            url = link['href']
            if url.startswith(base_url) and url not in all_urls:  # Only add unique links within 'features'
                all_urls.add(url)
                writer.writerow([url])
        
        # Find the "next" page link
        next_link = soup.find('a', text="Next")  # Adjust selector based on site design if needed
        if next_link:
