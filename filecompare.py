...
def file_similarity(file1, file2):
    ...
    with file1.open('r', encoding='utf-8', errors='ignore') as f1, \
...
files = [file for file in directory.iterdir() if file.is_file() and file.suffix in extensions_to_compare]
...
with output_file.open('w', encoding='utf-8') as md_file:
    if similar_files:
        ...
        for file1, file2, sim in similar_files:
            md_file.write(f"- **{file1.name}** and **{file2.name}** - Similarity: {sim:.2f}\n")
    else:
