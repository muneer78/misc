            ...
            for tag in post_tags:
                if tag:  # Skip empty tags
                    ...
                    if tag not in self.tag_collections:
                        self.tag_collections[tag] = {"slug": tag_slug, "posts": []}
  ...
  <body>
    ...
    def render_template(self, template, **kwargs) -> str:
        if self.jinja_env and hasattr(template, "render"):
            ...
            result = re.sub(
                r"{%\s*if\s+post_count\s*!=\s*1\s*%}s{%\s*endif\s*%}",
                "s" if kwargs.get("post_count", 1) != 1 else "",
                result,
            )

            # Handle basic if statements for total_tags pluralization
            result = re.sub(
                r"{%\s*if\s+total_tags\s*!=\s*1\s*%}s{%\s*endif\s*%}",
                "s" if kwargs.get("total_tags", 1) != 1 else "",
                result,
            )
            ...
            </a>""".format(slug=tag["slug"], name=tag["name"], count=tag["count"])
                    tag_items += tag_item + "\n        "
                result = re.sub(
                    r"{%\s*for\s+tag\s+in\s+tag_list\s*%}.*?{%\s*endfor\s*%}",
                    tag_items.rstrip(),
                    result,
                    flags=re.DOTALL,
                )
            ...
            <p><a href="/{post["url"]}" class="read-more">Read More →</a></p>
                ...
                result = re.sub(
                    r"{%\s*for\s+post\s+in\s+tag_posts\s*%}.*?{%\s*endfor\s*%}",
                    post_items.rstrip(),
                    result,
                    flags=re.DOTALL,
                )
            ...
            if "tag_posts" in kwargs:
                if kwargs["tag_posts"]:
                    result = re.sub(
                        r"{%\s*if\s+tag_posts\s*%}(.*?){%\s*else\s*%}.*?{%\s*endif\s*%}",
                        r"\1",
                        result,
                        flags=re.DOTALL,
                    )
            ...
            if "tag_list" in kwargs:
                if kwargs["tag_list"]:
                    result = re.sub(
                        r"{%\s*if\s+tag_list\s*%}(.*?){%\s*else\s*%}.*?{%\s*endif\s*%}",
                        r"\1",
                        result,
                        flags=re.DOTALL,
                    )
    ...
    def collect_posts(self):
        ...
        for file_path in markdown_files:
            ...
            if parsed:
                ...
                print(
                    f"✅ Successfully parsed: {post['title']} (collection: {collection_name})"
                )
            else:
    ...
    def generate_collection_indexes(self):
        ...
        for collection_name, collection_posts in self.collections.items():
            ...
            sorted_posts = sorted(
                collection_posts,
                key=lambda x: self.normalize_date(x["date"]),
                reverse=True,
            )
            ...
            collection_title = (
                collection_name.replace("_", " ").replace("-", " ").title()
            )
            ...
            print(
                f"✅ Generated: {collection_name}/index.html with {len(sorted_posts)} posts"
            )
    ...
    def generate_index(self):
        ...
        sorted_posts = sorted(
            posts_collection, key=lambda x: self.normalize_date(x["date"]), reverse=True
        )
        for post in sorted_posts:
            post["tags"] = self.list_tags(post.get("tags", []))
    ...
    def generate_archive(self):
        ...
        all_posts = sorted(
            filtered_posts, key=lambda x: self.normalize_date(x["date"]), reverse=True
        )
        ...
        filtered_collections = {
            name: posts
            for name, posts in self.collections.items()
        ...
        with open(archive_path, "w", encoding="utf-8") as f:
            f.write(rendered)
        print(
            f"✅ Generated: archive.html with {len(all_posts)} posts across {len(filtered_collections)} collections"
        )
    ...
    def generate_tags_index(self):
        ...
        rendered = self.render_template(
            tags_index_template, tag_list=tag_list, total_tags=len(tag_list)
        )
    ...
    def generate_rss_feed(self):
        ...
        sorted_posts = sorted(
            self.posts, key=lambda x: self.normalize_date(x["date"]), reverse=True
        )
        rss_posts = []
    ...
    def generate(self):
        ...
        print(
            f"📊 Generated {len(self.posts)} posts across {len(self.collections)} collections"
        )
