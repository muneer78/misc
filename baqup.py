...
_TEMPLATE_DIRECTORY = os.path.abspath(os.path.join(_BASE_DIRECTORY, "templates"))
...
def main():
    ...
    parser.add_argument(
        "--output_directory", default="./", help="Directory where to place backup data."
    )
    ...
    output_directory = os.path.join(_normalize_path(args.output_directory), "baqup")
...
def _run_backup(client, output_directory, root_folder_id):
    ...
    if root_folder_id:
        _descend_into_folder(
            root_folder_id, processed_folder_ids, client, output_directory, 0
        )
    else:
        _descend_into_folder(
            user["archive_folder_id"], processed_folder_ids, client, output_directory, 0
        )
        _descend_into_folder(
            user["desktop_folder_id"], processed_folder_ids, client, output_directory, 0
        )
...
def _descend_into_folder(
    folder_id, processed_folder_ids, client, output_directory, depth
):
    ...
    try:
        ...
        if e.code == 403:
            logging.warning(
                "%sSkipped over restricted folder %s.", "  " * depth, folder_id
            )
        else:
            logging.warning(
                "%sSkipped over folder %s due to unknown error %d.",
                "  " * depth,
                folder_id,
                e.code,
            )
        ...
        logging.warning(
            "%sSkipped over folder %s due to HTTP error %d.",
            "  " * depth,
            folder_id,
            e.code,
        )
...
def _backup_thread(thread, client, output_directory, depth):
    ...
    logging.info("%sBacking up thread %s (%s)...", "  " * depth, title, thread_id)
    ...
    if "html" in thread:
        ...
        for img in tree.iter("img"):
            ...
            image_filename = (
                blob_response.info().get("Content-Disposition").split('"')[-2]
            )
        ...
        document_output_path = os.path.join(output_directory, document_file_name)
        ...
        messages_html = _MESSAGES_TEMPLATE % {
            ...
            "body": "".join(
                [
                    ...
                    % {
                        "author_name": _escape(
                            _get_user(client, message["author_id"])["name"]
                        ),
...
def _get_conversation_threads(client):
    ...
    while True:
        chunk = client.get_recent_threads(
            max_updated_usec=max_updated_usec, count=50
        ).values()
        ...
        if chunk:
            ...
            if chunk_max_updated_usec == max_updated_usec:
                logging.warning(
                    "New chunk had the same max_updated_usec (%d) "
            ...
            break
        logging.info(
            "  Got %d threads, paged back to %s",
            len(threads),
            _format_usec(max_updated_usec),
        )
    threads.reverse()
    return threads
