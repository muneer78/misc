...
df['ARNameInt']=df["Account Relationship Member Name"].str.extract("(\d*\.?\d+)", expand=True)
...
df1 = df.groupby('ARInt',sort=False).apply(lambda x  : x.reset_index(drop=True).iloc[:x.reset_index(drop=True).ARInt.idxmax()+1,:])
