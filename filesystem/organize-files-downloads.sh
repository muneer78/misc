...
# Organize files
move_files "docs" txt pdf md csv
...
# Move any remaining files (except .toml and .lock) to temp folder
echo "Moving unrecognized file types to temp..."
find "$TARGET_DIR" -maxdepth 1 -type f ! -name "*.toml" ! -name "*.lock" -exec mv {} "$TARGET_DIR/temp" \; 2>/dev/null
